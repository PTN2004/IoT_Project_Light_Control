...
class HandLandMarksDetection():
    ...
    def detectHand(self, frame):
        ...
        if results.multi_hand_landmarks is not None:
            print("multi_hand_landmarks is not None")
...
def run(data_path, sign_img_path, split='val', resolution=(1280, 720)):
    hand_detector = HandLandMarksDetection()
...
if __name__ == "__main__":
    ...
    print(type(LABEL_TAG))
    data_path = './data2'
    sign_img_path = './sign_imgs2'
    run(data_path, sign_img_path, "train")
    run(data_path, sign_img_path, "val")
    run(data_path, sign_img_path, "test")
